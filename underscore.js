...
const myEmployeesAddresses = _.map(myEmployees, 'address');// use pluck to get addresses here.
...
// Bonus Points
// const totalByMonth = _.map(purchasesByMonth, function(x) { 
//   let a = [];
//   let b = [];
//   for ( let key in x ) {
//     a.push(x[key].price);
//   }
//   a.reduce((acc, cur) => b.push(acc + cur));
//   return b;
//  });
// console.log('PURSCHASES: ', purchasesByMonth);
// console.log('TOTALS: ', totalByMonth);
...
let fastFibonnaci = _.memoize(function(x) {
  return x < 2 ? x: fastFibonnaci(x - 1) + fastFibonnaci(x - 2);
...
let getJedi = _.memoize(function(x) {
  return axios.get(`https://swapi.co/api/people/${x}`);
